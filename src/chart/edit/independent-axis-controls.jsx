	...
	return (
		...
		>
			...
			<TextControl
				...
				onChange={(val) =>
					updateAttributeForDevice('independentAxis', { label: val })
				}
			/>
			<NumberControl
				label={__('Label Padding')}
				help={__('Determines the space between the label and the axis')}
				value={getCurrentValue('independentAxis', 'axisLabel')?.padding}
				disableUnits
				disabledUnits
				onChange={(val) => {
			...
			<NumberControl
				...
				value={
					getCurrentValue('independentAxis', 'axisLabel')?.maxWidth
				}
				disableUnits
				disabledUnits
				onChange={(val) => {
			...
			<SelectControl
				...
				onChange={(val) => {
					updateAttributeForDevice('independentAxis', { scale: val });
				}}
			/>
			...
			<Flex>
				<FlexItem>
					<NumberControl
						label={__('Minimum')}
						value={
							getCurrentValue('independentAxis', 'domain')?.[0]
						}
						disabled={
				...
				<FlexItem>
					<NumberControl
						label={__('Maximum')}
						value={
							getCurrentValue('independentAxis', 'domain')?.[1]
						}
						disabled={
							'stacked-bar' === chartType ||
							'bar' === chartType ||
			...
			<ToggleControl
				...
				checked={getCurrentValue('independentAxis', 'tickMarksActive')}
				onChange={(newValue) =>
					updateAttributeForDevice('independentAxis', {
			...
			<SelectControl
				...
				value={getCurrentValue('independentAxis', 'tickUnitPosition')}
				options={[
					{
						value: 'start',
						label: 'Start',
			...
			<ToggleGroupControl
				...
				isBlock
				value={
					getCurrentValue('independentAxis', 'tickLabels')?.fontSize
				}
			...
			<PanelRow>
				Determines the position of tick label relative to it’s parent
				node
			</PanelRow>
			<Flex>
				<FlexItem>
					<NumberControl
						label={__('DX')}
						value={
							getCurrentValue('independentAxis', 'tickLabels')?.dx
						}
						onChange={(value) => {
							const currentTickLabels =
								getCurrentValue(
				...
				<FlexItem>
					<NumberControl
						label={__('DY')}
						value={
							getCurrentValue('independentAxis', 'tickLabels')?.dy
						}
						onChange={(value) => {
							const currentTickLabels =
								getCurrentValue(
			...
			<AnglePickerControl
				...
				__nextHasNoMarginBottom
				value={getCurrentValue('independentAxis', 'tickLabels')?.angle}
				onChange={(val) => {
					const currentTickLabels =
						getCurrentValue('independentAxis', 'tickLabels') || {};
					updateAttributeForDevice('independentAxis', {
			...
			<NumberControl
				...
				value={
					getCurrentValue('independentAxis', 'tickLabels')?.maxWidth
				}
				disableUnits
				disabledUnits
				onChange={(val) => {
			...
			<ToggleGroupControl
				...
				isBlock
				value={
					getCurrentValue('independentAxis', 'tickLabels')?.textAnchor
				}
			...
			<PanelColorSettings
				...
				colorSettings={[
					...
					{
						value: getCurrentValue('independentAxis', 'tickLabels')
							?.fill,
						onChange: (value) => {
							const currentTickLabels =
								getCurrentValue(
			...
			<NumberControl
				...
				value={
					getCurrentValue('independentAxis', 'grid')?.strokeOpacity
				}
				disableUnits
				disabledUnits
				step={0.1}
			...
			<TextControl
				...
				value={
					getCurrentValue('independentAxis', 'grid')?.strokeDasharray
				}
				onChange={(val) => {
					const currentGrid =
						getCurrentValue('independentAxis', 'grid') || {};
