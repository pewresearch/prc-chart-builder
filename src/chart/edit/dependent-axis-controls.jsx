...
function DependentAxisControls({ attributes, setAttributes }) {
	...
	return (
		<div ref={panelRef}>
			...
			>
				<PanelRow>
					Dependent variables are properties that change in response
					to a change in another property. As such, the dependent axis
					is usually the y-axis.
				</PanelRow>
				<ToggleControl
					label="Axis active"
					help={
				...
				<NumberControl
					...
					value={
						getCurrentValue('dependentAxis', 'axisLabel')?.padding
					}
					disableUnits
					disabledUnits
					onChange={(val) => {
				...
				<NumberControl
					...
					value={
						getCurrentValue('dependentAxis', 'axisLabel')?.maxWidth
					}
					disableUnits
					disabledUnits
					onChange={(val) => {
				...
				<ToggleGroupControl
					...
					isBlock
					value={
						getCurrentValue('dependentAxis', 'tickLabels')?.fontSize
					}
				...
				<AnglePickerControl
					...
					__nextHasNoMarginBottom
					value={
						getCurrentValue('dependentAxis', 'tickLabels')?.angle
					}
					onChange={(val) => {
						const currentTickLabels =
							getCurrentValue('dependentAxis', 'tickLabels') ||
				...
				<NumberControl
					...
					value={
						getCurrentValue('dependentAxis', 'tickLabels')?.maxWidth
					}
					disableUnits
					disabledUnits
					onChange={(val) => {
				...
				<PanelColorSettings
					...
					colorSettings={[
						{
							value: getCurrentValue('dependentAxis', 'axis')
								?.stroke,
							onChange: (value) => {
								const currentAxis =
						...
						{
							value: getCurrentValue('dependentAxis', 'grid')
								?.stroke,
							onChange: (value) => {
								const currentGrid =
				...
				<NumberControl
					...
					value={
						getCurrentValue('dependentAxis', 'grid')?.strokeOpacity
					}
					step={0.1}
					min={0}
					max={1}
