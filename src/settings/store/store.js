...
export const store = createSettingsStore({
	...
	extraActions: {
		...
		/**
		 * Merge an uploaded theme into the in-editor draft without updating
		 * savedSettings so the page becomes dirty and GlobalSaveBar can
		 * persist via POST /theme. Omitted root keys (`config` / `palettes`)
		 * keep their current draft values.
		 *
		 * @param {ChartTheme} theme
		 */
		replaceTheme(theme) {
			return { type: 'REPLACE_THEME', payload: { theme } };
		},
	},
	extraReducer: (state, action) => {
		...
		if (action.type === 'REPLACE_THEME') {
			const { theme } = /** @type {{ theme: ChartTheme }} */ (
				action.payload
			);
			return applyReplaceTheme(state, theme);
		}
