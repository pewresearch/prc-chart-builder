...
class Attachments_Report_Integration {
	...
	private function build_chart_item( int $ref_id, int $post_id ): ?array {
		...
		$static_urls = $this->resolve_static_image_urls( $ref_id );
	 ...
	 */
	private function resolve_static_image_urls( int $chart_post_id ): array {
		$thumbnail_id = (int) get_post_thumbnail_id( $chart_post_id );
		$url          = $thumbnail_id ? (string) wp_get_attachment_url( $thumbnail_id ) : '';

		if ( ! $url ) {
			$io  = $this->get_chart_io_attributes( $chart_post_id );
	 ...
	 */
	private function get_chart_io_attributes( int $chart_post_id ): array {
		$content = (string) get_post_field( 'post_content', $chart_post_id );
		if ( '' === $content ) {
			return array();
		}

		$chart_block = $this->find_chart_block( parse_blocks( $content ) );
		if ( null === $chart_block ) {
			return array();
		}
