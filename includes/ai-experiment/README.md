...
Chart_AI_Ability::generate_chart()
  ...
  ▼
AiClient → Claude (claude-sonnet-5 default)
...
| `model`       | `string` (enum) | No       | Claude model ID; defaults to `claude-sonnet-5` |
...
- `model` is validated against `ALLOWED_MODELS`; falls back to `claude-sonnet-5`.
...
| `model`             | `'claude-sonnet-5'` | Selected Claude model                                             |
