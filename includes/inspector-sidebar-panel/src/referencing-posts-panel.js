...
export function ReferencingPostsPanel() {
	...
	useEffect(() => {
		//  Only fetch if we're on a chart post type and have a post ID
		if (postType !== 'chart' || !postId) {
			return;
		}
		...
		apiFetch({
			...
			.catch((fetchError) => {
				setError(
					fetchError.message ||
						__(
							'Failed to load referencing posts',
							'prc-chart-builder'
						)
				);
				setIsLoading(false);
		...
		>
			...
			{!isLoading && !error && referencingPosts.length === 0 && (
				<p className="empty-state">
					{__(
						'This chart is not currently referenced in any posts.',
						'prc-chart-builder'
					)}
				</p>
			)}
				...
				<div>
					<p className="posts-list-header">
						{referencingPosts.length === 1
							? __(
									'This chart is referenced in 1 post:',
									'prc-chart-builder'
								)
